...
//Jason: Adding script to start running the ADC
#define SHELLSCRIPT "\
#/bin/bash \n\
echo \"hello\" \n\
...
GstElement *g729_send(char *client_ip, int port){
	...
	char print_str[100] = {'\0'};
	//Jason: Im adding element rawaudioparse
	GstElement *pipeline, *audiosource, *rawaudioparse,*audioconvert , *audioresample ,*encoder, *rtp_pay, *udpsink;
	...
	//Jason: here i make the audio source the raw audio file from the adc
	audiosource = gst_element_factory_make ("filesrc",  NULL);
	//Jason: we will have to set location momentarily using g_object_set
	//Jason: Add rawaudioparse 
	rawaudioparse = gst_element_factory_make ("rawaudioparse", NULL);
	//Jason: add audioconvert
	audioconvert = gst_element_factory_make ("audioconvert", NULL);
	//Jason: add audioresample
	...
	//Check proper creation
	//Jason: add rawaudioparse to check
	if (!pipeline || !audiosource || !rawaudioparse || !audioconvert || !audioresample || !encoder || !rtp_pay || !udpsink) {
		g_printerr ("One G729_SEND element could not be created\n");
		return NULL;
	}

	//Set Properties of Objects
	//Jason:Im adding a line here to set location of raw audio file for streaming 
	// use-sink-caps=false format=pcm pcm-format=s16le sample-rate=*8000* num-channels=1
	g_object_set (G_OBJECT (audiosource), "location", "300hzSineTone.raw", NULL);
	
	//Jason: init elements properties of rawaudioparse
	//"use-sink-caps", "false","format","pcm","pcm-format", "s16le", "sample-rate", 8000, "num-channels",1,
	//gint SampleRate = 8000;
	//gint NumChannels = 1;
	int SampleRate = 860;
        int NumChannels = 1;
	gboolean Nah = FALSE;
	g_object_set (G_OBJECT (rawaudioparse),"use-sink-caps", Nah,"format",0,"sample-rate", SampleRate, "num-channels", NumChannels, NULL);
	//g_object_set (G_OBJECT (rawaudioparse),  "sample-rate", SampleRate, "num-channels",NumChannels, NULL);
	g_object_set (G_OBJECT (udpsink), 
								"host", client_ip,
								"port", port,
								 NULL);
	
	//Jason:try error checking see if the problem is raw audio parse
	//int samplerate;
	int *print;
	g_object_get (G_OBJECT (rawaudioparse), "sample-rate", &print, NULL);
	//g_print("\n\nRawaudioparse = ");
	//printf(samplerate);
	printf("sample rate %d \n",print); //'%s' string '%i' int
	//g_free(print);
	g_object_get (G_OBJECT (rawaudioparse), "num-channels", &print, NULL);
	...
	gst_bin_add_many (GST_BIN (pipeline),
		            audiosource, rawaudioparse ,audioconvert, audioresample, encoder, rtp_pay, udpsink, NULL);

	//Link Proper Capabilities
	//Jason:Here I am going to try to change the "rate" value to 860
	//first try with data rate 8000?
	//need to implement: rawaudioparse use-sink-caps=false format=pcm pcm-format=s16le sample-rate=860 num-channels=1
	GstCaps *raw_caps;
	...
	// I think I might have to change this to put raw audioparse in there
	//if (!link_elements_with_filter(audiosource, encoder, raw_caps)){
	//	puts("Caps not negotiated!");
	//	return NULL;
	//}

	//try and add link elements
	gst_element_link_many(audiosource, rawaudioparse ,audioconvert, audioresample, NULL);

	// OK IM GOING TO TRY TO MESS WITH THE LINKING OF THE ELEMENTS
        if (!link_elements_with_filter(audioresample, encoder, raw_caps)){
                puts("Caps not negotiated!");
                return NULL;
        }
