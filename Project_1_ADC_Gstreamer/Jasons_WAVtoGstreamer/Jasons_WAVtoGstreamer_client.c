...
GstElement *g729_send(char *client_ip, int port){
	...
	char print_str[100] = {'\0'};
	//Jason: Im adding element wavparse
	GstElement *pipeline, *audiosource, *wavparse,*audioconvert ,*encoder, *rtp_pay, *udpsink;
	...
	//Check proper creation
	//Jason: add wavparse to check
	if (!pipeline || !audiosource || !wavparse || !audioconvert || !encoder || !rtp_pay || !udpsink) {
	...
	g_object_set (G_OBJECT (audiosource), "location", "MRT48K_779_mono3.wav", NULL);
	...
	gst_bin_add_many (GST_BIN (pipeline),
		            audiosource, wavparse ,audioconvert, encoder, rtp_pay, udpsink, NULL);
	...
	gst_element_link_many(audiosource, wavparse ,audioconvert, NULL);

	// OK IM GOING TO TRY TO MESS WITH THE LINKING OF THE ELEMENTS
        if (!link_elements_with_filter(audioconvert, encoder, raw_caps)){
